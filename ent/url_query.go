...
func (uq *URLQuery) QueryPassword() *PasswordQuery {
...
func (uq *URLQuery) WithPassword(opts ...func(*PasswordQuery)) *URLQuery {
	query := (&PasswordClient{config: uq.config}).Query()
...
func (uq *URLQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Url, error) {
	...
	if query := uq.withPassword; query != nil {
		if err := uq.loadPassword(ctx, query, nodes, nil,
			func(n *Url, e *Password) { n.Edges.Password = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *URLQuery) loadPassword(ctx context.Context, query *PasswordQuery, nodes []*Url, init func(*Url), assign func(*Url, *Password)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Url)
	for i := range nodes {
		if nodes[i].password_urls == nil {
			continue
		}
		fk := *nodes[i].password_urls
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "password_urls" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
