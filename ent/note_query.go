...
func (nq *NoteQuery) QueryVault() *VaultQuery {
	query := (&VaultClient{config: nq.config}).Query()
...
func (nq *NoteQuery) WithVault(opts ...func(*VaultQuery)) *NoteQuery {
	query := (&VaultClient{config: nq.config}).Query()
...
func (nq *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := nq.withVault; query != nil {
		if err := nq.loadVault(ctx, query, nodes, nil,
...
func (nq *NoteQuery) loadVault(ctx context.Context, query *VaultQuery, nodes []*Note, init func(*Note), assign func(*Note, *Vault)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Note)
	for i := range nodes {
		if nodes[i].vault_notes == nil {
			continue
		}
		fk := *nodes[i].vault_notes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "vault_notes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
