...
// QueryWebauthnCredentials chains the current query on the "webauthnCredentials" edge.
...
// the "webauthnCredentials" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) loadWebauthnCredentials(ctx context.Context, query *WebAuthnCredentialQuery, nodes []*User, init func(*User), assign func(*User, *WebAuthnCredential)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_webauthn_credentials" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
